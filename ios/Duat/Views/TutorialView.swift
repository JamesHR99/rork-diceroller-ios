...
struct TutorialView: View {
    ...
    private var header: some View {
        HStack(spacing: 11) {
            DuatSymbol(art: DuatArt.classSigil(hero.id), fallback: hero.symbol,
    ...
    private var classDice: some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 6) {
                DuatIcon(name: DuatArt.Status.stamina, size: 15)
    ...
    static func pages(for hero: HeroClass) -> [BriefingPage] {
        [
            ...
            BriefingPage(
                ...
                title: hero.name,
                body: "\(hero.blurb)\n\n\(demigodTruth(hero))",
                art: DuatArt.classSigil(hero.id) ?? DuatArt.Status.champion,
...
private struct OrderDemoView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            HStack(spacing: 6) {
                DuatIcon(name: DuatArt.interactionRoll, size: 14)
    ...
    private var planRow: some View {
        ...
        return VStack(alignment: .leading, spacing: 4) {
            HStack(spacing: 6) {
                DuatIcon(name: DuatArt.chainConnector, size: 14)
                Text("THE PLAN")
                    .font(.system(size: 9, weight: .black))
                    .kerning(1)
                    .foregroundStyle(Theme.gold.opacity(0.85))
                        ...
                        if let face = hand.first(where: { $0.id == id }) {
                            ...
                            if index < plan.count - 1,
                               groups.contains(where: { $0.contains(index) && $0.contains(index + 1) }) {
                                DuatImage(name: DuatArt.chainConnector, width: 13, fit: .fit)
