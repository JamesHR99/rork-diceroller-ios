    ...
    var body: some View {
        Group {
            if let art, DuatArt.exists(art) {
                DuatImage(name: art, height: height, fit: .fit)
            } else {
...
struct PortraitMedallionView: View {
    ...
    private var isPainted: Bool { DuatArt.exists(art) }
...
struct HaloedSigilView: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 12.0, paused: !breathes)) { context in
            ...
            ZStack {
                DuatImage(name: DuatArt.trialHalo, height: diameter, fit: .fit)
