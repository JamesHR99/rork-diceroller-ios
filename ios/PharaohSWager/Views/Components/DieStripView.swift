...
struct FaceTileView: View {
    ...
    var body: some View {
        VStack(spacing: 2) {
            PharaohSWagerSymbol(art: face.kind.artName,
    ...
    private var revealed: some View {
        HStack(spacing: 3) {
            ForEach(Array(combo.required.enumerated()), id: \.offset) { index, ingredient in
                HStack(spacing: 1) {
                    PharaohSWagerSymbol(art: ingredient.pattern.artName,
...
struct RunStatusBar: View {
    ...
    var body: some View {
        HStack(spacing: compact ? 6 : 8) {
            pill(PharaohSWagerArt.Status.health, "heart.fill", "\(game.currentHP)/\(game.maxHP)", Theme.blood)
            pill(PharaohSWagerArt.currency, "circle.hexagongrid.fill", "\(game.gold)", Theme.gold)
            if let hero = game.heroClass, !compact {
                pill(PharaohSWagerArt.classSigil(hero.id) ?? "", hero.symbol, hero.name, hero.accent)
            }
        }
        .fixedSize()
    }
    ...
    private func pill(_ art: String, _ fallback: String, _ text: String, _ tint: Color) -> some View {
        HStack(spacing: 5) {
            PharaohSWagerSymbol(art: art, fallback: fallback, size: 18, tint: tint)
            Text(text)
