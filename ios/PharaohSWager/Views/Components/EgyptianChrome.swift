...
struct CartoucheView: View {
    ...
    var body: some View {
        Text(text.uppercased())
            ...
            .background {
                PharaohSWagerImage(name: PharaohSWagerArt.cartouche, fit: .stretch)
...
struct NightDialView: View {
    ...
    var body: some View {
        HStack(spacing: compact ? 7 : 9) {
            PharaohSWagerImage(name: gate.sunArt, height: compact ? 13 : 16, fit: .fit)
    ...
    private func mark(_ hour: Int) -> some View {
        ...
        return Group {
            if isNow {
                PharaohSWagerImage(name: PharaohSWagerArt.nightCurrent, height: markSize * 1.15, fit: .fit)
                    .colorMultiply(Theme.parchment)
            } else if cleared {
                PharaohSWagerImage(name: PharaohSWagerArt.nightCleared, height: markSize, fit: .fit)
                    .colorMultiply(tint)
            } else if isGateEnd {
                PharaohSWagerImage(name: PharaohSWagerArt.nightBoundary, height: markSize, fit: .fit)
                    .colorMultiply(Theme.parchmentDim)
                    .opacity(0.6)
            } else {
                PharaohSWagerImage(name: PharaohSWagerArt.nightHour, height: markSize * 0.85, fit: .fit)
                    .colorMultiply(Theme.parchmentDim)
                    .opacity(0.45)
            }
        }
