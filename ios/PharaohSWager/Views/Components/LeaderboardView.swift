...
struct LeaderboardView: View {
    ...
    private var header: some View {
        HStack(spacing: 6) {
            PharaohSWagerIcon(name: PharaohSWagerArt.utilityRecords, size: 14)
    ...
    private var emptyState: some View {
        VStack(spacing: 6) {
            PharaohSWagerImage(name: "duat_environment_star", height: 26, fit: .fit)
                .opacity(0.55)
    ...
    private func row(_ record: RunRecord, rank: Int) -> some View {
        ...
        return HStack(spacing: 8) {
            ...
            PharaohSWagerSymbol(art: PharaohSWagerArt.classSigil(record.classID),
            ...
            VStack(alignment: .leading, spacing: 1) {
                HStack(spacing: 4) {
                    ...
                    if record.sawDawn {
                        PharaohSWagerImage(name: "duat_environment_sun_bright", height: 12, fit: .fit)
                    }
                    if isNew {
                        Text("THIS RUN")
