...
struct GameOverView: View {
                ...
                VStack(spacing: 10) {
                    ...
                    Button {
                        ...
                        Text(won ? "Sail Again Tonight" : "Try Again Tomorrow Night")
                            ...
                            .frame(height: 48)
                            .background {
                                PharaohSWagerImage(name: PharaohSWagerArt.button(.primary, won ? .highlighted : .normal),
                                          fit: .stretch)
    ...
    private var endingScene: some View {
        ZStack {
            if won {
                ...
                ZStack {
                    PharaohSWagerImage(name: "duat_environment_sun_halo", height: 400, fit: .fit)
                        .opacity(0.55)
                    PharaohSWagerImage(name: "duat_environment_sun_bright", height: 190, fit: .fit)
    ...
    private var panelToggle: some View {
        HStack(spacing: 4) {
            toggleTab(title: "THIS VOYAGE", art: PharaohSWagerArt.utilityCodex, active: !showRecords) {
                showRecords = false
            }
            toggleTab(title: "RECORDS", art: PharaohSWagerArt.utilityRecords, active: showRecords) {
                showRecords = true
            }
        }
        .padding(3)
    ...
    private func toggleTab(title: String, art: String, active: Bool, action: @escaping () -> Void) -> some View {
        Button {
            ...
            HStack(spacing: 5) {
                PharaohSWagerIcon(name: art, size: 17)
                Text(title)
            ...
            .frame(height: 36)
            .background {
                PharaohSWagerImage(name: active ? PharaohSWagerArt.tabSelected : PharaohSWagerArt.tabUnselected, fit: .stretch)
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    private var statsPanel: some View {
        VStack(spacing: 8) {
            statRow(PharaohSWagerArt.nightCurrent, "hourglass", "Hours cleared",
                    "\(game.hoursCleared)/\(Voyage.totalHours)", Theme.gold)
            statRow(PharaohSWagerArt.Status.burn, "flame.fill", "Damage dealt", "\(game.totalDamage)", Theme.ember)
            statRow(PharaohSWagerArt.chainConnector, "link", "Combos landed", "\(game.totalCombos)", Theme.gold)
            statRow(PharaohSWagerArt.Status.critical, "sparkles", "Critical dice", "\(game.totalCrits)", Theme.gold)
            statRow(PharaohSWagerArt.classSigil(game.classID) ?? "", game.heroClass?.symbol ?? "person.fill",
            ...
            if let best = game.bestRecord {
                ...
                statRow(PharaohSWagerArt.Status.champion, "crown.fill", "Deepest ever",
    ...
    private func statRow(_ art: String, _ fallback: String, _ label: String,
        ...
        HStack {
            PharaohSWagerSymbol(art: art, fallback: fallback, size: 16, tint: tint)
                .frame(width: 22)
            Text(label)
