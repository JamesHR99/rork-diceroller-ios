...
struct ClassCardView: View {
    ...
    private var identityColumn: some View {
        VStack(spacing: 7) {
            ...
            PortraitMedallionView(art: CharacterArt.demigod(hero.id),
                                  ...
                                  diameter: 84)
                .background {
                    if let frame = PharaohSWagerArt.classFrame(hero.id) {
                        PharaohSWagerImage(name: frame, height: 104, fit: .fit)
    ...
    private func gearBlock(title: String, art: String?, symbol: String, die: Die?) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack(spacing: 6) {
                PharaohSWagerSymbol(art: art, fallback: symbol, size: 16, tint: hero.accent)
                Text(title)
    ...
    private func statPill(art: String?, icon: String, value: String, label: String, tint: Color) -> some View {
        VStack(spacing: 1) {
            PharaohSWagerSymbol(art: art, fallback: icon, size: 15, tint: tint)
            Text(value)
