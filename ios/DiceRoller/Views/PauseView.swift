...
struct PauseView: View {
    ...
    private var header: some View {
        VStack(spacing: 3) {
            DuatIcon(name: DuatArt.Status.judgement, size: 30)
                .opacity(0.9)
    ...
    private func pauseButton(
        ...
        art: String,
        fallback: String,
        tone: DuatArt.ButtonTone,
        rim: Color,
        action: @escaping () -> Void
    ) -> some View {
        Button(action: action) {
            HStack(spacing: 10) {
                DuatSymbol(art: art, fallback: fallback, size: 20, tint: rim)
                    .frame(width: 28)
    ...
    private func volumeRow(
        ...
        HStack(spacing: 9) {
            DuatSymbol(art: art, fallback: fallback, size: 15, tint: Theme.gold)
                .frame(width: 20)
